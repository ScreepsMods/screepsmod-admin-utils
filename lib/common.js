...
module.exports = (config) => {
  ...
  Object.assign(config.utils, {
    ...
    async reloadConfig () {
      ...
      try {
        ...
        if (gclToCPU) {
          config.utils.gclToCPU = gclToCPU
          if (maxCPU) {
            config.utils.maxCPU = maxCPU
          }
          if (baseCPU) {
            config.utils.baseCPU = baseCPU
          }
          if (stepCPU) {
            config.utils.stepCPU = stepCPU
          }
        }
      } catch (err) {
      }
    ...
    async setCPULimit (username, cpuLimit) {
      ...
      const usernameLower = username.toLowerCase()
      const user = await db.users.findOne({ usernameLower })
      if (!user) return 'Useranme not found'
      db.users.update({ _id: user._id }, { $set: { cpu: cpuLimit } })
      return `Updated maximum cpu to ${cpuLimit} for user ${user.username}`
    },
    enableGCLToCPU (maxCPU, baseCPU, stepCPU) {
      config.utils.gclToCPU = true
      if (maxCPU) {
        config.utils.maxCPU = maxCPU
      }
      if (baseCPU) {
    ...
    disableGCLToCPU () {
      delete config.utils.gclToCPU
      delete config.utils.maxCPU
      delete config.utils.baseCPU
      delete config.utils.stepCPU
  ...
  config.utils.getCPULimit._help = `getCPULimit(username)`
  config.utils.setCPULimit._help = `setCPULimit(username, value)`
  config.utils.enableGCLToCPU._help = `enableGCLToCPU(maxCPU, baseCPU, stepCPU) changes running state and does not persist between server restarts`
  config.utils.disableGCLToCPU._help = `disableGCLToCPU() changes running state and does not persist between server restarts`
