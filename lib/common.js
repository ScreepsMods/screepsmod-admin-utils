...
const utils = require(path.join(path.dirname(require.main.filename), '../lib/utils'))
...
module.exports = function common (config) {
  ...
  config.utils.removeNPCTerminals._help = `removeBots()`
  let funcs = []
  for (let k in config.utils) {
    let help = config.utils[k]._help
    funcs.push(help || `${k}()`)
  }
