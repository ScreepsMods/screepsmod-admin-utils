...
const requireOpts = {
  paths: require.resolve.paths('@screeps/backend/lib/utils.js')
}
requireOpts.paths.push(path.resolve(process.cwd(), 'node_modules'))
const utils = require(require.resolve('@screeps/backend/lib/utils.js', requireOpts))
...
module.exports = (config) => {
  const oConnect = config.common.storage._connect
  config.common.storage._connect = async () => {
    const ret = await oConnect()
    await config.utils.init()
    return ret
  }
  ...
  Object.assign(config.utils, {
    ...
    async init () {
      ...
      for (const hook in this.initHooks) {
        console.log(`Running Init Hook ${hook}`)
        await this.initHooks[hook](config)
      }
    },
    test () {
      console.log('Testing!')
