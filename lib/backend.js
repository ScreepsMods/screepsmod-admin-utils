...
module.exports = (config) => {
  config.backend.on('expressPreConfig', app => {
    ...
    app.get('/api/user/world-start-room', authroute.tokenAuth, async (req, res) => {
      ...
      if (!room) {
        const rooms = await db.rooms.find({ _id: /^[EW]\d*5[NW]\d*5$/ })
        if (rooms.length) {
    ...
    app.get('/api/experimental/pvp', async (req, res) => {
