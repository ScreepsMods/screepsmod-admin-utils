...
module.exports = (config) => {
  config.backend.on('expressPreConfig', app => {
    ...
    app.get('/api/version', errCatch(async (req, res) => {
      const users = await db.users.count({ $and: [{ active: { $ne: 0 } }, { cpu: { $gt: 0 } }, { bot: { $aeq: null } }] })
      const { welcomeText, customObjectTypes, historyChunkSize, socketUpdateThrottle, renderer } = config.backend
      const packageVersion = require('screeps').version
    ...
    app.get('/api/game/shards/info', errCatch(async (req, res) => {
