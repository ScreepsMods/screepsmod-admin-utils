const authroute = require.require('@screeps/backend/lib/game/api/auth')
...
module.exports = (config) => {
  ...
  config.backend.on('expressPreConfig', app => {
    ...
    config.common.storage.pubsub.subscribe('setConstants', (constants) => {
      for (const [k, v] of Object.entries(JSON.parse(constants))) {
    ...
    app.get('/api/game/room-objects', config.utils.errCatch(async (req, res) => {
      ...
      userIds.add('2')
      userIds.add('3')
      const users = indexById(await db.users.find({ _id: { $in: Array.from(userIds) } }, { username: 1, badge: 1 }))
