  ...
  config.engine.on('init', type => {
    ...
    if (type === 'main') {
      ...
      config.engine.on('mainLoopStage', stage => {
        ...
        lastStage = stage
        if (stage === 'finish') {
          config.utils.tickTiming = config.utils.tickTiming || []
          config.utils.tickTiming.push(times)
          config.utils.tickTiming.splice(100, 100)
          config.common.storage.pubsub.publish(pubsub.keys.TICK_TIMING, JSON.stringify(times))
        }
      })
    }
    if (type === 'processor') {
